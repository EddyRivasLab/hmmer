...
automatically. You also don't have to uncompress \mono{.gz} files;
...
\mono{-{}-nonull2} option (and if you're doing that, you may also want
...
domain profile using the \mono{fn3.sto} alignment (this happens
...
sequence \mono{7LESS\_DROME}, the \emph{Drosophila} Sevenless
...
The \mono{!} or \mono{?} symbol indicates whether this domain does
...
you can pass a command-line argument of \mono{-{}-seed 0} to
...
most alignment formats\sidenote{A2M format is an important exception!} HMMER is
...
down in an arbitrary order.\marginnote{By arbitrary HMMER convention,
...
\mono{hmmalign} currently has a, um, feature that you may dislike and
that I know about. Recall that HMMER only does local
alignments. Here, we know that we've provided full length globin
sequences, and \mono{globins4} is a full length globin profile. We'd
probably like \mono{hmmalign} to produce a global alignment. It can't
...
left or right. For example, look at the N-terminal \mono{g} in
\mono{MYG\_HORSE} above. HMMER is about 80\% confident that this
