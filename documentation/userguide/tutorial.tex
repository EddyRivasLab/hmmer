...
In rough order of importance, the HMMER programs are:
...
\monob{hmmer-makefmdb} & build nhmmer FM-index database from input seqfile\\
...
etc.), \mono{a2m|a3m}, \mono{phylip} (interleaved), \mono{phylips}
...
For more information on HMMER input files and formats, see the formats
...
alignments for thousands of sequence families.\marginnote{The Easel
  ...
  \mono{easel afetch} for extracting one alignment by name or accession
  from a Pfam file.}
...
consumption, not yours), you'd see something like:
   ...
   \xtinysreoutput{inclusions/hmmbuild-globins.out2}
...
internal repeats, and one of those repeats accidentally gives a weak
...
The difference between the two E-values is not apparent in the
\mono{7LESS\_DROME} example because in both cases, the size of the
search space is 1 sequence. There's a single sequence in the target
...
times we'd see a hit of \SFSmaxsc{} bits or better would now be \SFSievalue{} $\times$
...
probabilities currently not only include whether the residue is
...
sequences, you might wish to annotate a single sequence by
...
on our Harvard cluster):
   ...
   \vspace{1ex}
   \begin{fullwidth}
   \user{\% sbatch -J hmmbuild -e hmmbuild.err -o hmmbuild.out -p eddy -n 128 -t 6-00:00 --mem-per-cpu=4000 \textbackslash}\\
   \user{   --wrap="srun -n 128 --mpi=pmi2 ./hmmbuild --mpi Pfam-A.hmm Pfam-A.seed"}
   \end{fullwidth}
   \vspace{1em}
...
sequences are just padding for the alignment to accommodate those
...
most alignment formats\sidenote[][1ex]{A2M|A3M format is an important exception.} HMMER is
...
profile. The alignment will have ten insert columns, to accommodate the
...
internally produces the profile for that alignment\marginnote{Using
  ...
  build the profile with hmmbuild.}, then searches with that
...
\mono{hmmsearch}.  There is not currently an iterative DNA search
