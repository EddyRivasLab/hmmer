...
The Easel ``miniapps'' share the same pipe-reading ability.

Because the programs for profile HMM fetching (\mono{hmmfetch}) and
sequence fetching (\mono{easel sfetch}) can fetch any number of profiles
...
fly. For example:
   ...
   \vspace{1ex}
   \begin{fullwidth}
   \user{\% easel sfetch --index uniprot\_sprot.fasta} \\
   \user{\% cat mytargs.list | easel sfetch -f uniprot\_sprot.fasta - | hmmsearch globins4.hmm -} \\
   \end{fullwidth}
   \vspace{1ex}
...
\mono{easel sfetch} is reading its \mono{<namefile>} list of
...
Ditto for piping subsets of profiles. Supposing you have a copy of
Pfam in Pfam-A.hmm:
...
database:
   ...
   \user{\% cat mytargets.list | easel sfetch -f uniprot\_sprot.fasta - | hmmsearch many.hmms -}\\
   \end{fullwidth}
   \vspace{1ex}
...
reading the \emph{queries} from a stdin pipe, not the targets:
   ...
   \vspace{1ex}
   \user{\% cat mytargets.list | easel sfetch -f uniprot\_sprot.fasta > mytarget.seqs}\\
...
textual information and miscellaneous parameters in a roughly
...
All probability parameters are stored as negative natural log
...
nonzero integer, is the upper bound on the length at which an instance
...
\item [\monob{CONS <s>}] Consensus residue annotation flag; \mono{<s>}
  is either \mono{no} or \mono{yes} (case insensitive).  If
  ...
  the query sequence, and is all upper case. For models of multiple
  alignments, the consensus is the maximum likelihood residue at each
  position, and upper versus lower case indicates highly conserved
  vs. less conserved positions.  ``Highly conserved'' is defined as an
  emission probability for the consensus residue of $\geq$ an
...
\item [\monob{AC  <s>}]
        ...
        Often an alphabetical prefix indicating the database
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{A2M|A3M formats for multiple sequence alignments}

% Adapted from Easel documentation's esl_msafile_a2m.md by:
%  - including format_a2m.tex
%  - add the section header above
%  - revise first pp to be about HMMER.

HMMER and Easel programs are capable of writing alignments in A3M and
A2M formats. A2M (alignment to model) format was introduced as the
native format for the UC Santa Cruz SAM profile HMM software
package. It has two variants: a fully aligned format similar to
aligned FASTA that uses `.` characters to pad out insertion regions,
and a compact "dotless" format. The name "A3M" was later introduced by
the HH-suite software package to specifically mean the compact,
dotless variant, and today the full variant is rarely used.

To select A2M|A3M format, use the format code \mono{a2m} or
\mono{a3m}: for example, to reformat a Stockholm alignment to A3M:

\user{easel reformat a3m myali.sto}
...
\subsection{An example A2M|A3M file}
...
A2M|A3M do not support some special characters such as `*'
...
residues, lest they be read as FIMs. For this reason, HMMER and Easel convert
`O' residues to `X' when they write an amino acid alignment in A2M
format.
...
that has no reference annotation markup.  In this case, HMMER and
Easel internally generate a ``reasonable'' guess at consensus columns,
using essentially the same procedure that HMMER's \mono{hmmbuild}
program uses by default: sequence fragments (sequences $<$50\% of the
mean sequence length in the alignment overall) are ignored, and for
the remaining sequences, any column containing $\geq$ 50\% residues is
considered to be a consensus column.  Important: this dealigns regions
that get defined as insertions.
...
\item [\monob{cnt\_i}] Of the sequences in the file, the number that belong to
...
the hmmpgmd-format file, and post-process \mono{hmmpgmd} search results to 
apply those fields to the target sequence information.
Depending on the size of the map list, this might be easily achieved with a
...
The Easel miniapp \mono{easel reformat} is able to convert a file in unaligned
fasta format into an hmmpgmd format file, such as

\user{easel reformat --id\_map mydb.hmmpgmd.map hmmpgmd mydb.fasta > mydb.hmmpgmd}
...
file. You can download many standard score matrix files from NCBI at
...
\item The first data line is a header line, labeling each column with
  ...
  matrix has $20 \leq n \leq 27$; it must contain at least the 20 canonical
