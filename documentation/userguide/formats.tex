...
\item [\monob{STATS <s1> <s2> <f1> <f2>}] Statistical parameters
  ...
  statistics. \textbf{Mandatory.}\sidenote{Currently, HMMER's input
    parser treats these fields as optional: they don't have to be
    present just to read the HMM file. However, if \mono{STATS}
    calibration parameters aren't present, searches won't work (no
    hits will be found) because the HMMER3 acceleration filters depend
    on being able to calculate E-values. \mono{hmmbuild} always
    includes these parameters. If you're somehow creating your own HMM
    files outside of HMMER, you'll want to use \mono{p7\_Calibrate()}
    to determine these parameters in your model before writing it out.}
