...
Optionally, to compile and run a test suite\sidenote{The test suite
  requires that you have \mono{python3} and \mono{perl}, and that your
  \mono{python3} is version 3.5 or later. If your system lacks these
  dependencies, \mono{make check} will fail and tell you so. But HMMER
  is still fine, and you can install it -- running the test suite is
  optional!}:
...
delve deep into our codebase) requires Perl and Python (version 3.5 or
later) to be installed.  If you don't have them (which should be
rare), \mono{make check} won't work for you, but that's ok because
