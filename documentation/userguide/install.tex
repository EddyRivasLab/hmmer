...
\htmladdnormallink{ftp.genetics.wustl.edu/pub/eddy/hmmer/}
                  {ftp://ftp.genetics.wustl.edu/pub/eddy/hmmer/}
or 
\htmladdnormallink{hmmer.wustl.edu/}
...
\subsubsection{installing a precompiled binary distribution}

Alternatively, you can obtain a precompiled binary distribution of
HMMER from \htmladdnormallink{hmmer.wustl.edu/}
...
Intel x86/GNU Linux machines is \prog{hmmer.bin.intel-linux.tar.gz}.
...
Specifically, thanks to generous support from manufacturers, HMMER
distributions are tested and created on the following hardware and
operating systems:
\begin{itemize}
\item Intel x86/GNU Linux
\item Intel x86/FreeBSD
\item Intel x86/OpenBSD
\item Intel x86/Sun Solaris 
\item Intel Itanium2/GNU Linux
\item Intel Itanium2/HP-UX
\item Apple Macintosh OS/X
\item Silicon Graphics/IRIX
\item Sun Sparc/Solaris 
\item Compaq Alpha/Tru64
\item Compaq Alpha/GNU Linux
\item Hewlett Packard HP-UX
\item IBM/AIX
\end{itemize}

Additional community-contributed ports to other systems are made
available from
\htmladdnormallink{hmmer.wustl.edu/}{http://hmmer.wustl.edu/}, if they
are sent to me.

Running the testsuite (\prog{make check}) requires that you have
...
\subsection{Running the \prog{configure} script}
...
If you turn on threads support, by default, HMMER programs will use
\emph{all} available CPUs. Especially on a shared machine, you may
want to rein it in, and leave some CPU for everyone else. There are
...
\htmladdnormallink{www.csm.ornl.gov/pvm/}{http://www.csm.ornl.gov/pvm/}.

\subsubsection{turning on LFS support for files $>$2 GB}

HMMER can access large files ($>$ 2 GB) even on normal 32-bit
operating systems, using Large File Summit (LFS).  Most modern UNIX
systems have LFS extensions. To enable this optional code, use
the \prog{--enable-lfs} flag:
...
The \prog{configure} script will automatically detect whether you're
on a machine that supports Altivec instructions or not, so it doesn't
...
instrumented by the \prog{ccmalloc} memory checking library.  You have
to have \prog{ccmalloc} installed.  \prog{ccmalloc} can be obtained
from Armin Biere at ETH Zurich, at
\htmladdnormallink{www.inf.ethz.ch/personal/biere/projects/ccmalloc/}
...
\subsection{The \prog{config.h} header file}
...
BLASTMAT directory first, then in the current directory.
...
Start the PVM by typing\\ \user{pvm hostfile} (where ``hostfile'' is
the name of your hostfile) on the master. Make sure all the nodes
started properly by typing \user{conf} at the PVM console prompt.
Type \user{quit} to exit from the PVM console, which leaves the PVM
running in the background.  You should only need to start PVM
once. (We have a PVM running continuously on our network right now,
waiting for HMMER jobs.)
...
\htmladdnormallink{pfam.wustl.edu}{http://pfam.wustl.edu/}.  However,
