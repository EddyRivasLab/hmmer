...
Microsoft Windows.\sidenote{Windows 10 includes a Unix-compatible bash
  command shell, which should work fine. For older Windows, there are
  ...
  which is freely available.}  We test most extensively on Intel/Linux
and on Apple OS/X.
...
2007.\sidenote{ If your platform does not support one of these vector
  instruction sets, the configure script will revert to an unoptimized
  implementation called the ``dummy'' implementation. \textbf{The
    dummy implementation is two orders of magnitude slower.} It will
  enable you to see H3's scientific features on a much wider range of
  processors, but \textbf{is not suited for production work}. If
  you're a packager, please don't package a compiled dummy
  implementation.}
