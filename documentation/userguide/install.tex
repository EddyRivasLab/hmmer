...
Optionally, to compile and run a test suite:\sidenote{Our self test
  ...
  \mono{python3} in your environment, the \mono{./configure} step
  detects that, and \mono{make check} gives you an error message.
  If you then do something clever to get \mono{python3} into your
  environment (\mono{module load python}, say, on one of our HPC
  systems), you'll need to \mono{./configure} again.}
...
\mono{python3}\sidenote{Specifically, our \mono{./configure} script
  tests for you having a \mono{python3} command, for Python 3, in case
  of mixed Python 2 versus 3 environments.}
