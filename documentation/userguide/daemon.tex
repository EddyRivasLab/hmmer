...
Figures \ref{fig:search-stats-serialize}--\ref{fig:alidisplay-serialize} show how the data structures used in sending search results back to clients are serialized.  The text streams the client uses to send commands to the daemon do not require serialization, as text streams do not contain pointers and are sequences of byte-wide characters that are not affected by endianness.  The \mono{HMMD\_SEARCH\_STATS} structure, shown in Figure \ref{fig:search-stats-serialize}, does not contain any pointers, so can be serialized by simply converting each field in the structure to network byte order before sending and reversing the conversion on the receiving side.  
