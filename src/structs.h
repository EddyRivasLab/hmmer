...
struct hit_s {
  ...
  double evalue;		/* E-value of the hit               */
...
/* Structure: tophit_s
 * 
 * Array of high scoring hits. The goal is to efficiently 
 * keep the top H hits in a large database search.
 * 
 * There's some fun algorithmic issues here. To keep the top H
 * scores as we sweep through a database, we keep an array
 * of H+x top scores. When that array is filled, we use a
 * fast O(H+x) partition sort to put the top H scores in the
 * first H slots of the array, and the Hth highest score in
 * elem H-1. Then we can start reaccumulating another x high
 * scores that are greater than elem H-1; resort when we fill
 * the array; and carry on with this until we have all the scores.
 */
struct tophit_s {
  struct hit_s **hit;           /* array of ptrs to top scoring hits        */
  int            H;		/* # to keep; i.e. H=100, keep top 100      */
  int            A;		/* # to keep with alignments                */
  int            alloc;		/* H+x; tradeoff memory/speed               */
  int            pos;		/* current position in the hit array        */
  double         best;          /* evalue must be <= best to be included    */
  int            sorts;		/* how many sorts have been done (debugging)*/
};
