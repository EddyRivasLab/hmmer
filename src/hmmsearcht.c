...
//        add_id_length(id_length_list, seqid, block->list[i].L);
        seqid++;
        prev_char_cnt += block->list[i].n; /* capture the total # chars in one block, so the next one knows how many chars in preceding blocks*/
      ...
      for (i = 0; i < dnablock->count; ++i)
      {
          ...
          for (k = 0; k < orfblock->count; ++k)
          {
              ...
              */
              ...
              //sprintf(dbsq_aa->orfid, "xorf%" PRId64 "", dnablock->prev_res+k);
