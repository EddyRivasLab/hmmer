...
P7_ALIDISPLAY *
p7_alidisplay_Create_avx(const P7_TRACE *tr, int which, const P7_OPROFILE *om, const ESL_SQ *sq)
{
  ...
  for (z = z1; z <= z2; z++) 
    {
      ...
      case p7T_MG:
        ...
        else if (p7_oprofile_FGetEmission_avx(om, k, x) > 1.0)               ad->mline[z-z1] = '+'; /* >1 not >0; om has odds ratios, not scores */
        else                                                             ad->mline[z-z1] = ' ';
