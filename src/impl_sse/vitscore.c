...
{
  ESL_GETOPTS    *go      = p7_CreateDefaultApp("vitscore_benchmark", options, 1, argc, argv, banner, usage);
...
main(int argc, char **argv)
{
  ESL_GETOPTS    *go   = p7_CreateDefaultApp("vitscore_utest", options, 0, argc, argv, banner, usage);
  ESL_RANDOMNESS *r    = esl_randomness_CreateFast(esl_opt_GetInteger(go, "-s"));
  ESL_ALPHABET   *abc  = NULL;
  P7_BG          *bg   = NULL;
