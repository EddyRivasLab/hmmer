...
{
  ESL_GETOPTS    *go      = p7_CreateDefaultApp("fwdback_benchmark", options, 1, argc, argv, banner, usage);
...
main(int argc, char **argv)
{
  ESL_GETOPTS    *go   = p7_CreateDefaultApp("fwdback_utest", options, 0, argc, argv, banner, usage);
  ESL_RANDOMNESS *r    = esl_randomness_CreateFast(esl_opt_GetInteger(go, "-s"));
  ESL_ALPHABET   *abc  = NULL;
  P7_BG          *bg   = NULL;
...
main(int argc, char **argv)
{
  ESL_GETOPTS    *go      = p7_CreateDefaultApp("fwdback_example", options, 2, argc, argv, banner, usage);
  char           *hmmfile = esl_opt_GetArg(go, 1);
