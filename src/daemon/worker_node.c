...
#define CHUNK_FRACTION 10  // each chunk is 1/(2^^CHUNK_FRACTION) of the work
...
*/
  ...
  // Just create a new hitlist.  Assumes that some other routine has grabbed the pointer to the hitlist
      ...
      case SEQUENCE_SEARCH:
        ...
        while(stop == 0){
          ...
          if((start != -1) && (start <= end)){
            ...
            //uint64_t  chunk_end = start + ((end - start) >> CHUNK_FRACTION);  // do 1/8 of remaining work or until we get a hit
            ...
            // grab the sequence Id and length out of the shard
              ...
              int status = p7_engine_Overthruster_tophalf(workernode->thread_state[my_id].engine, the_sequence, L, workernode->thread_state[my_id].om, workernode->thread_state[my_id].bg, P);  
...
//                workernode->sequences_processed[seq_id] = 1;
                ...
                if(start <= end){
                  ...
                  int status = p7_engine_Overthruster_bottomhalf(workernode->thread_state[my_id].engine, the_sequence, L, workernode->thread_state[my_id].om, workernode->thread_state[my_id].bg, P);  
...
void worker_thread_process_chunk_amino_vs_hmm_db(P7_DAEMON_WORKERNODE_STATE *workernode, uint32_t my_id, uint64_t start_index, uint64_t chunk_length){
  ...
  for (i = 0; i < chunk_length; i++){
    ...
    if(p7_engine_Compare_Sequence_HMM(workernode->thread_state[my_id].engine, workernode->compare_sequence, workernode->compare_L, gm, om, bg)){
      ...
      the_entry = workernode->thread_state[my_id].engine->empty_hit_pool;
      workernode->thread_state[my_id].engine->empty_hit_pool = workernode->thread_state[my_id].engine->empty_hit_pool->next;
      // grab the first entry off the free list now that we know there is one
