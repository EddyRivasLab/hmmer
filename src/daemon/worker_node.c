...
// init the global queue
  ...
  workernode->backend_pool = p7_backend_pool_Create((workernode->num_threads +3) << BACKEND_INCREMENT_FACTOR);
...
//      printf("%s, %f\n", the_hit->name, ((float)((end_time.tv_sec * 1000000 + (end_time.tv_usec)) - (start_time.tv_sec * 1000000 + start_time.tv_usec)))/1000000.0);
 ...
 //printf("Thread %d switching back to frontend\n", my_id);
...
//printf("Worker %d received HMM\n", my_rank);
        ...
        om = p7_oprofile_Create(gm->M, gm->abc, eng->hw->simd); 
