...
p7_oprofile_Create(int allocM, const ESL_ALPHABET *abc)
{
  ...
  ESL_ALLOC(om->rb_mem, sizeof(vector unsigned char) * nqb  * abc->Kp          +15);
  ESL_ALLOC(om->rw_mem, sizeof(vector signed short)  * nqw  * abc->Kp          +15);
  ESL_ALLOC(om->tw_mem, sizeof(vector signed short)  * nqw  * p7O_NTRANS       +15);
  ESL_ALLOC(om->rf_mem, sizeof(vector float)         * nqf  * abc->Kp          +15);
  ESL_ALLOC(om->tf_mem, sizeof(vector float)         * nqf  * p7O_NTRANS       +15);

  ESL_ALLOC(om->rb, sizeof(vector unsigned char *) * abc->Kp);
  ESL_ALLOC(om->rw, sizeof(vector signed short *)  * abc->Kp);
  ESL_ALLOC(om->rf, sizeof(vector float *)         * abc->Kp);

  /* align vector memory on 16-byte boundaries */
  om->rb[0] = (vector unsigned char *) (((unsigned long int) om->rb_mem + 15) & (~0xf));
  om->rw[0] = (vector signed short *)  (((unsigned long int) om->rw_mem + 15) & (~0xf));
  om->tw    = (vector signed short *)  (((unsigned long int) om->tw_mem + 15) & (~0xf));
  om->rf[0] = (vector float *)         (((unsigned long int) om->rf_mem + 15) & (~0xf));
  om->tf    = (vector float *)         (((unsigned long int) om->tf_mem + 15) & (~0xf));
...
p7_oprofile_Copy(P7_OPROFILE *om1)
{
  ...
  /* +15 is for manual 16-byte alignment */
  ESL_ALLOC(om2->rb_mem, sizeof(vector unsigned char) * nqb  * abc->Kp    +15);	
  ESL_ALLOC(om2->rw_mem, sizeof(vector signed short)  * nqw  * abc->Kp    +15);
  ESL_ALLOC(om2->tw_mem, sizeof(vector signed short)  * nqw  * p7O_NTRANS +15);
  ESL_ALLOC(om2->rf_mem, sizeof(vector float)         * nqf  * abc->Kp    +15);
  ESL_ALLOC(om2->tf_mem, sizeof(vector float)         * nqf  * p7O_NTRANS +15);

  ESL_ALLOC(om2->rb, sizeof(vector unsigned char *) * abc->Kp);
  ESL_ALLOC(om2->rw, sizeof(vector signed short *)  * abc->Kp);
  ESL_ALLOC(om2->rf, sizeof(vector float *)         * abc->Kp);

  /* align vector memory on 16-byte boundaries */
  om2->rb[0] = (vector unsigned char *) (((unsigned long int) om2->rb_mem + 15) & (~0xf));
  om2->rw[0] = (vector signed short *)  (((unsigned long int) om2->rw_mem + 15) & (~0xf));
  om2->tw    = (vector signed short *)  (((unsigned long int) om2->tw_mem + 15) & (~0xf));
