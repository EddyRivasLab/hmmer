...
P7_OMX *
p7_omx_Create_avx512(int allocM, int allocL, int allocXL)
{
  P7_OMX  *ox     = NULL;
  int      i;
  int      status;
  ...
  ox->ncells   = (int64_t) ox->allocR * (int64_t) ox->allocQ4_avx512 * 16;      /* # of DP cells allocated, where 1 cell contains MDI */
...
P7_OMX *
p7_omx_Create_test_all_simd(int allocM, int allocL, int allocXL)
{
  P7_OMX  *ox     = NULL;
  int      i;
  int      status;
...
int
p7_omx_GrowTo_avx512(P7_OMX *ox, int allocM, int allocL, int allocXL)
{
  void   *p;
  ...
  int64_t ncells = (int64_t) (allocL+1) * (int64_t) nqf_avx512 * 16;
  int     reset_row_pointers = FALSE;
  int     i;
  int     status;
 
  /* If all possible dimensions are already satisfied, the matrix is fine */
  if (ox->allocQ4_avx512*16 >= allocM && ox->validR > allocL && ox->allocXR >= allocXL+1) return eslOK;
  ...
  if (reset_row_pointers)
    {
      ...
      ox->validR = ESL_MIN( ox->ncells / (nqf_avx512 * 16), ox->allocR);
...
int
p7_omx_GrowTo_test_all_simd(P7_OMX *ox, int allocM, int allocL, int allocXL)
{
  void   *p;
...
void
p7_omx_Destroy_avx512(P7_OMX *ox)
{
  if (ox == NULL) return;
...
int
p7_omx_FDeconvert_avx512(P7_OMX *ox, P7_GMX *gx)
{
  ...
  for (i = 0; i <= ox->L; i++)
    {
      ...
      for (q = 0; q < Q; q++)
	{
	  u.v = MMO(ox->dpf_avx512[i],q);  for (r = 0; r < 16; r++) { k = (Q*r)+q+1; if (k <= ox->M) MMX(i, (Q*r)+q+1) = u.p[r]; }
	  u.v = DMO(ox->dpf_avx512[i],q);  for (r = 0; r < 16; r++) { k = (Q*r)+q+1; if (k <= ox->M) DMX(i, (Q*r)+q+1) = u.p[r]; }
	  u.v = IMO(ox->dpf_avx512[i],q);  for (r = 0; r < 16; r++) { k = (Q*r)+q+1; if (k <= ox->M) IMX(i, (Q*r)+q+1) = u.p[r]; }
	}
