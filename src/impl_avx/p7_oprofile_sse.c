/* Intel SSE implementations of SIMD-specific routines related to the P7_OPROFILE structure
 */
#include <p7_config.h>
 ...
 * Synopsis:  Allocate an optimized profile structure, Intel SSE version.
 * Incept:    NPC, 5/20/2024 [House of Deer]
 *
 ...
 * Synopsis:  Frees an optimized profile structure, Intel SSE version.
 * Incept:    NPC, 5/20/2024 [House of Deer]
 */
void
 ...
 */
P7_OPROFILE *
p7_oprofile_Copy_sse(P7_OPROFILE *om1)
{
  int           x, y;
  int           status;
 ...
 */
int p7_oprofile_sf_Conversion_sse(P7_OPROFILE *om)
{
  int     M   = om->M;		/* length of the query                                          */
  int     nq  = p7O_NQB(M);     /* segment length; total # of striped vectors needed            */
 ...
 */
int p7_oprofile_mf_Conversion_sse(const P7_PROFILE *gm, P7_OPROFILE *om)
{
 ...
 * throws <eslEINVAL> if <om> hasn't been allocated properly.
 */
int p7_oprofile_vf_Conversion_sse(const P7_PROFILE *gm, P7_OPROFILE *om)
{
  int     M   = gm->M;		/* length of the query                                          */
  int     nq  = p7O_NQW(M);     /* segment length; total # of striped vectors needed            */
  int     x;			/* counter over residues                                        */
 ...
 */
int
p7_oprofile_fb_Conversion_sse(const P7_PROFILE *gm, P7_OPROFILE *om)
{
  int     M   = gm->M;		/* length of the query                                          */
  int     nq  = p7O_NQF(M);     /* segment length; total # of striped vectors needed            */
  int     x;			/* counter over residues                                        */
 ...
 * Incept:    SRE, 5/20/24 [House of Deer]
 *
 ...
 *            optimized profile into a flat array, Intel SSE version
 *
...
/* Function:  p7_oprofile_GetSSVEmissionScoreArray_sse()
 * Synopsis:  Retrieve sSV residue emission scores from an optimized
 *            profile into an array, Intel SSE version
 *
 ...
 */
int
p7_oprofile_Dump_sse(FILE *fp, const P7_OPROFILE *om)
{
  int status;
 ...
 * Synopsis:  Compare two optimized profiles for equality, Intel SSE version.
 * Incept:    NPC 5/20/24 [House of Deer]
 *
