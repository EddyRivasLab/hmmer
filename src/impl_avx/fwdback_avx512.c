...
{
   __m512 mpv, dpv, ipv;   /* previous row values                                       */
   __m512 sv;		   /* temp storage of 1 curr row value in progress              */
   __m512 dcv;		   /* delayed storage of D(i,q+1)                               */
   __m512 xEv;		   /* E state: keeps max for Mk->E as we go                     */
   __m512 xBv;		   /* B state: splatted vector of B[i-1] for B->Mk calculations */
  __m512   zerov;		   /* splatted 0.0's in a vector                                */
  ...
  int Q       = p7O_NQF_AVX512(om->M);	   /* segment length: # of vectors                              */
  ...
  __m512 *rp;			   /* will point at om->rfv[x] for residue x[i]                 */
  __m512 *tp;			   /* will point into (and step thru) om->tfv                   */

  /* Initialization. */
  ox->M  = om->M;
  ox->L  = L;
  ox->last_written_by = avx512;
  ox->has_own_scales = TRUE; 	/* all forward matrices control their own scalefactors */
  zerov  = _mm512_setzero_ps();
  for (q = 0; q < Q; q++)
...
#if eslDEBUGLEVEL > 0
  if (ox->debugging) p7_omx_DumpFBRow_avx512(ox, TRUE, 0, 9, 5, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=0, width=8, precision=5*/
#endif

  for (i = 1; i <= L; i++)
    {
      dpp   = dpc;                      
      dpc   = ox->dpf_avx512[do_full * i];     /* avoid conditional, use do_full as kronecker delta */
      rp    = om->rfv_avx512[dsq[i]];
      tp    = om->tfv_avx512;
      dcv   = _mm512_setzero_ps();
      xEv   = _mm512_setzero_ps();
      xBv   = _mm512_set1_ps(xB);

      /* Right shifts by 4 bytes. 4,8,12,x becomes x,4,8,12.  Shift zeros on. */
      mpv   = esl_avx512_rightshiftz_float(MMO(dpp,Q-1));
      dpv   = esl_avx512_rightshiftz_float(DMO(dpp,Q-1));
      ipv   = esl_avx512_rightshiftz_float(IMO(dpp,Q-1));
      
      for (q = 0; q < Q; q++)
	{
	  /* Calculate new MMO(i,q); don't store it yet, hold it in sv. */
	  sv   =                _mm512_mul_ps(xBv, *tp);  tp++;
	  sv   = _mm512_add_ps(sv, _mm512_mul_ps(mpv, *tp)); tp++;
	  sv   = _mm512_add_ps(sv, _mm512_mul_ps(ipv, *tp)); tp++;
	  sv   = _mm512_add_ps(sv, _mm512_mul_ps(dpv, *tp)); tp++;
	  sv   = _mm512_mul_ps(sv, *rp);                  rp++;
	  ...
	  IMO(dpc,q) = _mm512_add_ps(sv, _mm512_mul_ps(ipv, *tp)); tp++;
	}	  
      ...
      for (q = 0; q < Q; q++) 
	{
	  DMO(dpc,q) = _mm512_add_ps(dcv, DMO(dpc,q));	
	  dcv        = _mm512_mul_ps(DMO(dpc,q), *tp); tp++; /* extend DMO(q), so we include M->D and D->D paths */
	}
	...
	{			/* Slightly parallelized version, but which incurs some overhead */
	  for (j = 1; j < 4; j++)
		...
		{ /* using cmpgt below tests if DD changed any DMO(q) *without* conditional branch */
		  sv         = _mm512_add_ps(dcv, DMO(dpc,q));	
		  cv         = _mm512_kor(cv, _mm512_cmp_ps_mask(sv, DMO(dpc,q), 14)); // 14 = greater than 
      ...
      /* Add D's to xEv */
      for (q = 0; q < Q; q++) xEv = _mm512_add_ps(DMO(dpc,q), xEv);
      ...
      if (xE > 1.0e4)	/* that's a little less than e^10, ~10% of our dynamic range */
	{
	  ...
	  xJ  = xJ / xE;
	  xB  = xB / xE;
	  xEv = _mm512_set1_ps(1.0 / xE);
	  for (q = 0; q < Q; q++)
	    {
	      MMO(dpc,q) = _mm512_mul_ps(MMO(dpc,q), xEv);
	      DMO(dpc,q) = _mm512_mul_ps(DMO(dpc,q), xEv);
	      IMO(dpc,q) = _mm512_mul_ps(IMO(dpc,q), xEv);
	    }
...
#if eslDEBUGLEVEL > 0
      if (ox->debugging) p7_omx_DumpFBRow_avx512(ox, TRUE, i, 9, 5, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=i, width=9, precision=5*/
#endif
...
{
 __m512 mpv, ipv, dpv;      /* previous row values                                       */
 __m512 mcv, dcv;           /* current row values                                        */
 __m512 tmmv, timv, tdmv;   /* tmp vars for accessing rotated transition scores          */
 __m512 xBv;		      /* collects B->Mk components of B(i)                         */
 __m512 xEv;	              /* splatted E(i)                                             */
  __m512   zerov;		      /* splatted 0.0's in a vector                                */
  float    xN, xE, xB, xC, xJ;	      /* special states' scores                                    */
  ...
  __m512  *dpc;                       /* current DP row                                            */
  __m512  *dpp;			      /* next ("previous") DP row                                  */
  __m512  *rp;			      /* will point into om->rfv[x] for residue x[i+1]             */
  __m512  *tp;		              /* will point into (and step thru) om->tfv transition scores */

  /* initialize the L row. */
  bck->M = om->M;
  bck->L = L;
  bck->last_written_by = avx512;
  bck->has_own_scales = FALSE;	/* backwards scale factors are *usually* given by <fwd> */
  dpc    = bck->dpf_avx512[L * do_full];
  xJ     = 0.0;
  xB     = 0.0;
  xN     = 0.0;
  xC     = om->xf[p7O_C][p7O_MOVE];      /* C<-T */
  xE     = xC * om->xf[p7O_E][p7O_MOVE]; /* E<-C, no tail */
  xEv    = _mm512_set1_ps(xE); 
  zerov  = _mm512_setzero_ps();  
  dcv    = zerov;		/* solely to silence a compiler warning */
  for (q = 0; q < Q; q++) MMO(dpc,q) = DMO(dpc,q) = xEv;
  for (q = 0; q < Q; q++) IMO(dpc,q) = zerov;
#if eslDEBUGLEVEL > 0
  if (bck->debugging) p7_omx_DumpFBRow_avx512(bck, TRUE, L, 9, 4, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=L, width=9, precision=4*/
#endif
  /* init row L's DD paths, 1) first segment includes xE, from DMO(q) */
  tp  = om->tfv_avx512 + 8*Q - 1;	                        /* <*tp> now the [4 8 12 x] TDD quad         */
  dpv = esl_avx512_leftshiftz_float(DMO(dpc,Q-1));             
  
  for (q = Q-1; q >= 0; q--)
    {
      dcv        = _mm512_mul_ps(dpv, *tp);      tp--;
      DMO(dpc,q) = _mm512_add_ps(DMO(dpc,q), dcv);
      dpv        = DMO(dpc,q);
    }
...
#if eslDEBUGLEVEL > 0
  if (bck->debugging) p7_omx_DumpFBRow_avx512(bck, TRUE, L, 9, 4, xE, xN, xJ, x\
B, xC);  /* logify=TRUE, <rowi>=L, width=9, precision=4*/
#endif
  /* now MD init */
  tp  = om->tfv_avx512 + 7*Q - 3;	                        /* <*tp> now the [4 8 12 x] Mk->Dk+1 quad    */
  dcv = esl_avx512_leftshiftz_float(DMO(dpc,0));
  for (q = Q-1; q >= 0; q--)
    {
      MMO(dpc,q) = _mm512_add_ps(MMO(dpc,q), _mm512_mul_ps(dcv, *tp)); tp -= 7;
      dcv        = DMO(dpc,q);
    }
#if eslDEBUGLEVEL > 0
  if (bck->debugging) p7_omx_DumpFBRow_avx512(bck, TRUE, L, 9, 4, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=L, width=9, precision=4*/
  ...
  if (fwd->xmx[L*p7X_NXCELLS+p7X_SCALE] > 1.0)
    {
      ...
      xEv = _mm512_set1_ps(1.0 / fwd->xmx[L*p7X_NXCELLS+p7X_SCALE]);
      for (q = 0; q < Q; q++) {
	      MMO(dpc,q) = _mm512_mul_ps(MMO(dpc,q), xEv);
	      DMO(dpc,q) = _mm512_mul_ps(DMO(dpc,q), xEv);
	      IMO(dpc,q) = _mm512_mul_ps(IMO(dpc,q), xEv);
  ...
  for (i = L-1; i >= 1; i--)	/* backwards stride */
    {
       ...
       */
      dpc = bck->dpf_avx512[i     * do_full];
      dpp = bck->dpf_avx512[(i+1) * do_full];
      rp  = om->rfv_avx512[dsq[i+1]] + Q-1; /* <*rp> is now the [4 8 12 x] match emission quad */
      tp  = om->tfv_avx512 + 7*Q - 1;	     /* <*tp> is now the [4 8 12 x] TII transition quad  */

      /* rightshift the first transition quads */
      tmmv = esl_avx512_leftshiftz_float(om->tfv_avx512[1]);
      timv = esl_avx512_leftshiftz_float(om->tfv_avx512[2]);
      tdmv = esl_avx512_leftshiftz_float(om->tfv_avx512[3]);

      mpv = _mm512_mul_ps(MMO(dpp,0), om->rfv_avx512[dsq[i+1]][0]); /* precalc M(i+1,k+1) * e(M_k+1, x_{i+1}) */
      mpv = esl_avx512_leftshiftz_float(mpv);

      xBv = zerov;
      for (q = Q-1; q >= 0; q--)     /* backwards stride */
	{
	  ipv = IMO(dpp,q); /* assumes emission odds ratio of 1.0; i+1's IMO(q) now free */
	  IMO(dpc,q) = _mm512_add_ps(_mm512_mul_ps(ipv, *tp), _mm512_mul_ps(mpv, timv));   tp--;
	  DMO(dpc,q) =                                  _mm512_mul_ps(mpv, tdmv); 
	  mcv        = _mm512_add_ps(_mm512_mul_ps(ipv, *tp), _mm512_mul_ps(mpv, tmmv));   tp-= 2;
	  
	  mpv        = _mm512_mul_ps(MMO(dpp,q), *rp);  rp--;  /* obtain mpv for next q. i+1's MMO(q) is freed  */
	  MMO(dpc,q) = mcv;
	  ...
	  xBv = _mm512_add_ps(xBv, _mm512_mul_ps(mpv, *tp)); tp--;
	}
      ...
      xEv = _mm512_set1_ps(xE);	/* splat */


      /* phase 3: {MD}->E paths and one step of the D->D paths */
      tp  = om->tfv_avx512 + 8*Q - 1;	/* <*tp> now the [4 8 12 x] TDD quad */
      dpv = _mm512_add_ps(DMO(dpc,0), xEv);
      dpv = esl_avx512_leftshiftz_float(dpv);
      for (q = Q-1; q >= 0; q--)
	{
	  dcv        = _mm512_mul_ps(dpv, *tp); tp--;
	  DMO(dpc,q) = _mm512_add_ps(DMO(dpc,q), _mm512_add_ps(dcv, xEv));
	  dpv        = DMO(dpc,q);
	  MMO(dpc,q) = _mm512_add_ps(MMO(dpc,q), xEv);
	}
      ...
      for (j = 1; j < 4; j++)	/* three passes: we've already done 1 segment, we need 4 total */
	{
      ...
      dcv = esl_avx512_leftshiftz_float(DMO(dpc,0));
      tp  = om->tfv_avx512 + 7*Q - 3;	/* <*tp> is now the [4 8 12 x] Mk->Dk+1 quad */
      for (q = Q-1; q >= 0; q--)
	{
	  MMO(dpc,q) = _mm512_add_ps(MMO(dpc,q), _mm512_mul_ps(dcv, *tp)); tp -= 7;
	  dcv        = DMO(dpc,q);
      ...
      if (bck->xmx[i*p7X_NXCELLS+p7X_SCALE] > 1.0)
	{
	  ...
	  xBv = _mm512_set1_ps(1.0 / bck->xmx[i*p7X_NXCELLS+p7X_SCALE]);
	  for (q = 0; q < Q; q++) {
	    MMO(dpc,q) = _mm512_mul_ps(MMO(dpc,q), xBv);
	    DMO(dpc,q) = _mm512_mul_ps(DMO(dpc,q), xBv);
	    IMO(dpc,q) = _mm512_mul_ps(IMO(dpc,q), xBv);
	  }
...
#if eslDEBUGLEVEL > 0
      if (bck->debugging) p7_omx_DumpFBRow_avx512(bck, TRUE, i, 9, 4, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=i, width=9, precision=4*/
#endif
    } /* thus ends the loop over sequence positions i */

  /* Termination at i=0, where we can only reach N,B states. */
  dpp = bck->dpf_avx512[1 * do_full];
  tp  = om->tfv_avx512;          /* <*tp> is now the [1 5 9 13] TBMk transition quad  */
  rp  = om->rfv_avx512[dsq[1]];  /* <*rp> is now the [1 5 9 13] match emission quad   */
  xBv = zerov;
  for (q = 0; q < Q; q++)
    {
      mpv = _mm512_mul_ps(MMO(dpp,q), *rp);  rp++;
      mpv = _mm512_mul_ps(mpv,        *tp);  tp += 7;
  ...
  if (bck->debugging) p7_omx_DumpFBRow_avx512(bck, TRUE, 0, 9, 4, bck->xmx[p7X_E], bck->xmx[p7X_N],  bck->xmx[p7X_J], bck->xmx[p7X_B],  bck->xmx[p7X_C]);	/* logify=TRUE, <rowi>=0, width=9, precision=4*/
#endif
