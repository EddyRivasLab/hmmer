...
/* Function:  p7_oprofile_Create_test_sse_avx()
 * Synopsis:  Allocate an optimized profile structure for both SSE and AVX ISAs to enable testing.
...
/* Function:  p7_oprofile_Sizeof_test_sse_avx()
 * Synopsis:  Return the allocated size of a <P7_OPROFILE>, test version with both SSE and AVX ISAs.
 * Incept:    NPC, 5/23/24 [The Cushy Office]
 *
 ...
 */
size_t
p7_oprofile_Sizeof_test_sse_avx(P7_OPROFILE *om)
{
 ...
 * Synopsis:  Return the allocated size of a <P7_OPROFILE>, for AVX ISA.
 * Incept:    NPC, 5/23/24 [The Cushy Office]
 *
 * Purpose:   Returns the allocated size of a <P7_OPROFILE>,
 *            in bytes.
 */
size_t
p7_oprofile_Sizeof_avx(P7_OPROFILE *om)
{
  size_t n   = 0;
...
p7_oprofile_UpdateMSVEmissionScores_avx(P7_OPROFILE *om, P7_BG *bg, float *fwd_emissions, float *sc_arr)
{
 ...
 p7_Die("p7_oprofileUpdateMSVEmissionScores_avx has not been implemented yet.  If you are not Nick C., you should complain to him about this.\n");
 ...
 int p7_oprofile_mf_Conversion_avx(const P7_PROFILE *gm, P7_OPROFILE *om)
{
  ...
  p7_oprofile_sf_Conversion_avx(om);
...
p7_oprofile_Convert_test_sse_avx(const P7_PROFILE *gm, P7_OPROFILE *om)
{
  ...
  /* calling both the SSE and AVX conversion routines re-computes some scalar values, but 
     not enough to be a big deal for testing */
  if ((status =  p7_oprofile_mf_Conversion_sse(gm, om)) != eslOK) return status;   /* MSVFilter()'s information     */
  if ((status =  p7_oprofile_mf_Conversion_avx(gm, om)) != eslOK) return status;   /* MSVFilter()'s information     */
  if ((status =  p7_oprofile_vf_Conversion_sse(gm, om)) != eslOK) return status;   /* ViterbiFilter()'s information */
  if ((status =  p7_oprofile_vf_Conversion_avx(gm, om)) != eslOK) return status;   /* ViterbiFilter()'s information */
  if ((status =  p7_oprofile_fb_Conversion_sse(gm, om)) != eslOK) return status;   /* ForwardFilter()'s information */
  if ((status =  p7_oprofile_fb_Conversion_avx(gm, om)) != eslOK) return status;   /* ForwardFilter()'s information */
...
{
  int     nq  = p7O_NQF_AVX(om->M);     /* # of striped vectors needed            */
  int i, j;
...
{
  int x, q, z, k;
  union { __m256i v; uint8_t i[32]; } tmp; /* used to align and read simd minivectors           */
  int      M   = om->M;    /* length of the query                                          */
  int      K   = om->abc->Kp;
  int      nq  = p7O_NQB_AVX(M);     /* segment length; total # of striped vectors needed            */
...
p7_oprofile_GetFwdEmissionScoreArray_avx(const P7_OPROFILE *om, float *arr )
{
  ...
  for (x = 0; x < K; x++) {
      for (q = 0, k = 1; q < nq; q++, k++) {
        tmp.v = esl_avx_logf(om->rfv_avx[x][q]);
        for (z = 0; z < 8; z++)
          if (  (K * (k+z*nq) + x) < cell_cnt)
...
p7_oprofile_GetFwdEmissionArray_avx(const P7_OPROFILE *om, P7_BG *bg, float *arr )
{
  ...
  int      Kp  = om->abc->Kp;
  int      K   = om->abc->K;
  int      nq  = p7O_NQF_AVX(M);     /* segment length; total # of striped vectors needed            */
...
{
  int     M   = om->M;		/* length of the query                                          */
  int     nq  = p7O_NQW_AVX(M);     /* segment length; total # of striped vectors needed            */
  int     x;			/* counter over residues                                        */
  int     q;			/* q counts over total # of striped vectors, 0..nq-1            */
  int     k;			/* the usual counter over model nodes 1..M                      */
  int     kb;			/* possibly offset base k for loading om's TSC vectors          */
...
{
  int Q4  = p7O_NQF_AVX(om1->M);
  int Q8  = p7O_NQW_AVX(om1->M);
  int Q16 = p7O_NQB_AVX(om1->M);
  int q, r, x, y;
  ...
  for (x = 0; x < om1->abc->Kp; x++)
    for (q = 0; q < Q16; q++)
      {
	a16.v = om1->rbv_avx[x][q]; b16.v = om2->rbv_avx[x][q];
	for (r = 0; r < 32; r++) if (a16.c[r] != b16.c[r]) ESL_FAIL(eslFAIL, errmsg, "comparison failed: rb[%d] elem %d", q, r);
      }
  ...
  for (x = 0; x < om1->abc->Kp; x++)
    for (q = 0; q < Q8; q++)
      {
	a8.v = om1->rwv_avx[x][q]; b8.v = om2->rwv_avx[x][q];
	for (r = 0; r < 16; r++) if (a8.w[r] != b8.w[r]) ESL_FAIL(eslFAIL, errmsg, "comparison failed: rw[%d] elem %d", q, r);
      }
  for (q = 0; q < 8*Q16; q++)
    {
      a8.v = om1->twv_avx[q]; b8.v = om2->twv_avx[q];
      for (r = 0; r < 16; r++) if (a8.w[r] != b8.w[r]) ESL_FAIL(eslFAIL, errmsg, "comparison failed: tw[%d] elem %d", q, r);
    }
  for (x = 0; x < p7O_NXSTATES; x++)
    for (y = 0; y < p7O_NXTRANS; y++)
  ...
  for (x = 0; x < om1->abc->Kp; x++)
    for (q = 0; q < Q4; q++)
      {
	a4.v = om1->rfv_avx[x][q]; b4.v = om2->rfv_avx[x][q];
	for (r = 0; r < 8; r++) if (esl_FCompare_old(a4.x[r], b4.x[r], tol) != eslOK)  ESL_FAIL(eslFAIL, errmsg, "comparison failed: rf[%d] elem %d", q, r);
      }
  for (q = 0; q < 8*Q4; q++)
    {
      a4.v = om1->tfv_avx[q]; b4.v = om2->tfv_avx[q];
      for (r = 0; r < 8; r++) if (a4.x[r] != b4.x[r]) ESL_FAIL(eslFAIL, errmsg, "comparison failed: tf[%d] elem %d", q, r);
    }
  for (x = 0; x < p7O_NXSTATES; x++)
