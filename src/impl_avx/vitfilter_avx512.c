...
p7_ViterbiFilter_avx512(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, P7_OMX *ox, float *ret_sc)
{
  ...
  int Q        = p7O_NQW_AVX512(om->M);   /* segment length: # of vectors                              */
  __m512i *dp  = ox->dpw_avx512[0];	   /* using {MDI}MX(q) macro requires initialization of <dp>    */
  __m512i *rsc;			   /* will point at om->ru[x] for residue x[i]                  */
  __m512i *tsc;			   /* will point into (and step thru) om->tu                    */

  __m512i negInfv;

  /* Check that the DP matrix is ok for us. */
  if (Q > ox->allocQ8_avx512)                                 ESL_EXCEPTION(eslEINVAL, "DP matrix allocated too small");
  if (om->mode != p7_LOCAL && om->mode != p7_UNILOCAL) ESL_EXCEPTION(eslEINVAL, "Fast filter only works for local alignment");
  ox->M   = om->M;

  /* -infinity is -32768 */
 
  /* negInfv = 16-byte vector, 14 0 bytes + 2-byte value=-32768, for an OR operation. */
  negInfv = _mm512_setzero_si512();
  __m256i dummy1 = _mm256_setzero_si256();
  dummy1 = _mm256_insert_epi16(dummy1, -32768, 0);
  
  negInfv = _mm512_inserti64x4(negInfv, dummy1, 0);
  /* Initialization. In unsigned arithmetic, -infinity is -32768
   */
  for (q = 0; q < Q; q++)
    MMXo(q) = IMXo(q) = DMXo(q) = _mm512_set1_epi16(-32768);
  xN   = om->base_w;
...
#if eslDEBUGLEVEL > 0
  if (ox->debugging) p7_omx_DumpVFRow_avx512(ox, 0, xE, 0, xJ, xB, xC); /* first 0 is <rowi>: do header. second 0 is xN: always 0 here. */
#endif

  for (i = 1; i <= L; i++)
    {
      rsc   = om->rwv_avx512[dsq[i]];
      tsc   = om->twv_avx512;
       ...
       */
      mpv = MMXo(Q-1);  
      mpv= esl_avx512_rightshift_int16(mpv, negInfv);

      dpv = DMXo(Q-1);
      dpv = esl_avx512_rightshift_int16(dpv, negInfv);

      ipv = IMXo(Q-1);
      ipv = esl_avx512_rightshift_int16(ipv, negInfv);
      ...
      for (q = 0; q < Q; q++)
      {
        ...
        IMXo(q)= _mm512_max_epi16 (sv, _mm512_adds_epi16(ipv, *tsc)); tsc++;
      }
      ...
      if (Dmax + om->ddbound_w > xB) 
	{
	  ...
	  dcv = esl_avx512_rightshift_int16(dcv, negInfv);
	  tsc = om->twv_avx512 + 7*Q;	/* set tsc to start of the DD's */
	  for (q = 0; q < Q; q++) 
	    {
	      DMXo(q) = _mm512_max_epi16(dcv, DMXo(q));	
	      dcv     = _mm512_adds_epi16(DMXo(q), *tsc); tsc++;
	    }
	  ...
	  do {
	    ...
	    for (q = 0; q < Q; q++) 
	      {
		if (! esl_avx512_any_gt_epi16(dcv, DMXo(q))) break;
		DMXo(q) = _mm512_max_epi16(dcv, DMXo(q));	
		dcv     = _mm512_adds_epi16(DMXo(q), *tsc);   tsc++;
	      }	    
	  } while (q == Q);
	...
	{
	  DMXo(0) = esl_avx512_rightshift_int16(dcv, negInfv);
	}
	  
#if eslDEBUGLEVEL > 0
      if (ox->debugging) p7_omx_DumpVFRow_avx512(ox, i, xE, 0, xJ, xB, xC);   
#endif
