...
{
  __m128 mpv, dpv, ipv;   /* previous row values                                       */
  __m128 sv;		   /* temp storage of 1 curr row value in progress              */
  __m128 dcv;		   /* delayed storage of D(i,q+1)                               */
  __m128 xEv;		   /* E state: keeps max for Mk->E as we go                     */
  __m128 xBv;		   /* B state: splatted vector of B[i-1] for B->Mk calculations */
...
#if eslDEBUGLEVEL > 0
  if (ox->debugging) p7_omx_DumpFBRow_sse(ox, TRUE, 0, 9, 5, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=0, width=8, precision=5*/
#endif
...
#if eslDEBUGLEVEL > 0
      if (ox->debugging) p7_omx_DumpFBRow_sse(ox, TRUE, i, 9, 5, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=i, width=9, precision=5*/
#endif
...
{
__m128 mpv, ipv, dpv;      /* previous row values                                       */
__m128 mcv, dcv;           /* current row values                                        */
__m128 tmmv, timv, tdmv;   /* tmp vars for accessing rotated transition scores          */
__m128 xBv;		      /* collects B->Mk components of B(i)                         */
__m128 xEv;	              /* splatted E(i)                                             */
  __m128   zerov;		      /* splatted 0.0's in a vector                                */
  float    xN, xE, xB, xC, xJ;	      /* special states' scores                                    */
...
#if eslDEBUGLEVEL > 0
  if (bck->debugging) p7_omx_DumpFBRow_sse(bck, TRUE, L, 9, 4, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=L, width=9, precision=4*/
#endif
...
#if eslDEBUGLEVEL > 0
      if (bck->debugging) p7_omx_DumpFBRow_sse(bck, TRUE, i, 9, 4, xE, xN, xJ, xB, xC);	/* logify=TRUE, <rowi>=i, width=9, precision=4*/
#endif
  ...
  if (bck->debugging) p7_omx_DumpFBRow_sse(bck, TRUE, 0, 9, 4, bck->xmx[p7X_E], bck->xmx[p7X_N],  bck->xmx[p7X_J], bck->xmx[p7X_B],  bck->xmx[p7X_C]);	/* logify=TRUE, <rowi>=0, width=9, precision=4*/
#endif
