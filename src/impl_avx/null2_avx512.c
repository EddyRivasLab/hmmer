 ...
 *            <om>, <pp> are AVX-512 optimized versions of the profile
...
p7_Null2_ByExpectation_avx512(const P7_OPROFILE *om, const P7_OMX *pp, float *null2)
{
   ...
   */
  memcpy(pp->dpf_avx512[0], pp->dpf_avx512[1], sizeof(__m512) * 3 * Q);
  ...
  for (i = 2; i <= Ld; i++)
    {
      for (q = 0; q < Q; q++)
	{
	  pp->dpf_avx512[0][q*3 + p7X_M] = _mm512_add_ps(pp->dpf_avx512[i][q*3 + p7X_M], pp->dpf_avx512[0][q*3 + p7X_M]);
	  pp->dpf_avx512[0][q*3 + p7X_I] = _mm512_add_ps(pp->dpf_avx512[i][q*3 + p7X_I], pp->dpf_avx512[0][q*3 + p7X_I]);
	}
  ...
  norm = 1.0 / (float) Ld;
  sv   = _mm512_set1_ps(norm);
  for (q = 0; q < Q; q++)
    {
      pp->dpf_avx512[0][q*3 + p7X_M] = _mm512_mul_ps(pp->dpf_avx512[0][q*3 + p7X_M], sv);
      pp->dpf_avx512[0][q*3 + p7X_I] = _mm512_mul_ps(pp->dpf_avx512[0][q*3 + p7X_I], sv);
    }
  XMXo(0,p7X_N) *= norm;
  XMXo(0,p7X_C) *= norm;
  XMXo(0,p7X_J) *= norm;
  ...
  for (x = 0; x < om->abc->K; x++)
    {
      sv = _mm512_setzero_ps();
      rp = om->rfv_avx512[x];
      for (q = 0; q < Q; q++)
	{
	  sv = _mm512_add_ps(sv, _mm512_mul_ps(pp->dpf_avx512[0][q*3 + p7X_M], *rp)); rp++;
	  sv = _mm512_add_ps(sv,            pp->dpf_avx512[0][q*3 + p7X_I]);              /* insert odds implicitly 1.0 */
	  //	  sv = _mm_add_ps(sv, _mm_mul_ps(pp->dpf[0][q*3 + p7X_I], *rp)); rp++; 
	}
      esl_avx512_hsum_ps(sv, &(null2[x]));
      null2[x] += xfactor;
    }
 ...
 *            <om>, <wrk> are AVX-512 optimized versions of the profile
 *            and the residue posterior probability matrix. See 
 *            <p7_GNull2_ByTrace()>  documentation.
 */
int
p7_Null2_ByTrace_avx512(const P7_OPROFILE *om, const P7_TRACE *tr, int zstart, int zend, P7_OMX *wrk, float *null2)
{
  ...
  for (q = 0; q < Q; q++)
    {
      wrk->dpf_avx512[0][q*3 + p7X_M] = _mm512_setzero_ps();
      wrk->dpf_avx512[0][q*3 + p7X_I] = _mm512_setzero_ps();
    }
  XMXo(0,p7X_N) =  0.0;
  XMXo(0,p7X_C) =  0.0;
  XMXo(0,p7X_J) =  0.0;
  ...
  for (q = 0; q < Q; q++)
    {
      wrk->dpf_avx512[0][q*3 + p7X_M] = _mm512_mul_ps(wrk->dpf_avx512[0][q*3 + p7X_M], sv);
      wrk->dpf_avx512[0][q*3 + p7X_I] = _mm512_mul_ps(wrk->dpf_avx512[0][q*3 + p7X_I], sv);
    }
  XMXo(0,p7X_N) *= norm;
  XMXo(0,p7X_C) *= norm;
  XMXo(0,p7X_J) *= norm;
  ...
  for (x = 0; x < om->abc->K; x++)
    {
      ...
      for (q = 0; q < Q; q++)
	{
	  sv = _mm512_add_ps(sv, _mm512_mul_ps(wrk->dpf_avx512[0][q*3 + p7X_M], *rp)); rp++;
	  sv = _mm512_add_ps(sv,            wrk->dpf_avx512[0][q*3 + p7X_I]); /* insert emission odds implicitly 1.0 */
