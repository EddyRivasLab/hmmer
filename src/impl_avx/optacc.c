...
static int
OptimalAccuracy_dispatcher(const P7_OPROFILE *om, const P7_OMX *pp, P7_OMX *ox, float *ret_e);
...
int (*p7_OptimalAccuracy)(const P7_OPROFILE *om, const P7_OMX *pp, P7_OMX *ox, float *ret_e) =
  OptimalAccuracy_dispatcher;

static int
OptimalAccuracy_dispatcher(const P7_OPROFILE *om, const P7_OMX *pp, P7_OMX *ox, float *ret_e)
{
#ifdef P7_TEST_ALL_SIMD
  p7_OptimalAccuracy = p7_OptimalAccuracy_test_all;
  return p7_OptimalAccuracy_test_all(om, pp, ox, ret_e);
#endif

#ifdef P7_TEST_SSE_AVX
  p7_OptimalAccuracy = p7_OptimalAccuracy_test_sse_avx;
  return p7_OptimalAccuracy_test_sse_avx(om, pp, ox, ret_e); 
#endif

#ifdef eslENABLE_AVX512  // Fastest first.
  if (esl_cpu_has_avx512())
    {
      p7_OptimalAccuracy = p7_OptimalAccuracy_avx512;
      return p7_OptimalAccuracy_avx512(om, pp, ox, ret_e);
    }
#endif
  ...
  if (esl_cpu_has_avx())
    {
      p7_OptimalAccuracy = p7_OptimalAccuracy_avx;
      return p7_OptimalAccuracy_avx(om, pp, ox, ret_e);
    }
#endif
  ...
  p7_Die("OptimalAccuracy_dispatcher found no vector implementation - that shouldn't happen.");
  return eslFAIL;
}
...
static int
oatrace_dispatcher(const P7_OPROFILE *om, const P7_OMX *pp, const P7_OMX *ox, P7_TRACE *tr);
// SSV Filter function pointer.  Starts out pointing to the dispatcher, gets updated to
// point at the best-available SIMD implementation on first call
...
{
#ifdef P7_TEST_ALL_SIMD
  p7_OATrace = p7_OATrace_test_all;
  return p7_OATrace_test_all(om, pp, ox, tr);
#endif

#ifdef P7_TEST_SSE_AVX
  p7_OATrace = p7_OATrace_test_sse_avx;
  return p7_OATrace_test_sse_avx(om, pp, ox, tr); 
#endif

#ifdef eslENABLE_AVX512  // Fastest first.
  if (esl_cpu_has_avx512())
    {
      p7_OATrace = p7_OATrace_avx512;
      return p7_OATrace_avx512(om, pp, ox, tr);
    }
#endif
  ...
  if (esl_cpu_has_avx())
    {
      p7_OATrace = p7_OATrace_avx;
      return p7_OATrace_avx(om, pp, ox, tr);
    }
#endif
  ...
  p7_Die("oatrace_dispatcher found no vector implementation - that shouldn't happen.");
  return eslFAIL;
}
