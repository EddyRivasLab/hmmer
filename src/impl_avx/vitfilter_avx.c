...
p7_ViterbiFilter_avx(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, P7_OMX *ox, float *ret_sc)
{
  ...
  int Q        = p7O_NQW_AVX(om->M);   /* segment length: # of vectors                              */
  __m256i *dp  = ox->dpw_avx[0];	   /* using {MDI}MX(q) macro requires initialization of <dp>    */
...
#if eslDEBUGLEVEL > 0
  if (ox->debugging) p7_omx_DumpVFRow_avx(ox, 0, xE, 0, xJ, xB, xC); /* first 0 is <rowi>: do header. second 0 is xN: always 0 here. */
#endif
...
#if eslDEBUGLEVEL > 0
      if (ox->debugging) p7_omx_DumpVFRow_avx(ox, i, xE, 0, xJ, xB, xC);   
#endif
