...
p7_OptimalAccuracy_avx(const P7_OPROFILE *om, const P7_OMX *pp, P7_OMX *ox, float *ret_e)
{
  ...
  __m256 *ppp;			   /* quads in the <pp> posterior probability matrix            */
  __m256 *tp;			   /* quads in the <om->tfv> transition scores                  */
  ...
  for (i = 1; i <= pp->L; i++)
    {
      ...
      dpc = ox->dpf_avx[i];   	/* current DP row in OA matrix  */
      ppp = pp->dpf_avx[i];		/* current row in the posterior probabilities per position */
      tp  = om->tfv_avx;		/* transition probabilities */
      dcv = infv;
      xEv = infv;
      xBv = _mm256_set1_ps(XMXo(i-1, p7X_B));

      mpv = _mm256_blend_ps(esl_avx_rightshiftz_float(MMO(dpp,Q-1)), infv, 1 );  /* Right shifts by 4 bytes. 4,8,12,x becomes x,4,8,12. */
      dpv = _mm256_blend_ps(esl_avx_rightshiftz_float(DMO(dpp,Q-1)), infv, 1 );
      ipv = _mm256_blend_ps(esl_avx_rightshiftz_float(IMO(dpp,Q-1)), infv, 1 ); 
      for (q = 0; q < Q; q++)
	{
	  sv  =                _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), xBv);  tp++; // 14 = greater than
	  sv  = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), mpv)); tp++;
	  sv  = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), ipv)); tp++;
	  sv  = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), dpv)); tp++;
	  ...
	  dcv = _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), sv); tp++;

	  sv         =                _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), mpv);   tp++;
	  sv         = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), ipv));  tp++;
       ...
       */
      dcv = _mm256_blend_ps(esl_avx_rightshiftz_float(dcv), infv, 1 );
      tp  = om->tfv_avx + 7*Q;	/* set tp to start of the DD's */
      for (q = 0; q < Q; q++)
	{
	  DMO(dpc, q) = _mm256_max_ps(dcv, DMO(dpc, q));
	  dcv         = _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), DMO(dpc,q));   tp++;
	}
      ...
      for (j = 1; j < 8; j++)
	{
	  ...
	  for (q = 0; q < Q; q++)
	    {
	      DMO(dpc, q) = _mm256_max_ps(dcv, DMO(dpc, q));
	      dcv         = _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, 14), dcv);   tp++;
	    }
	}
      ...
      /* Specials */
      esl_avx_hmax_ps(xEv, &(XMXo(i,p7X_E)));
...
select_m(const P7_OPROFILE *om, const P7_OMX *ox, int i, int k)
{
  ...
  int     q     = (k-1) % Q;		/* (q,r) is position of the current DP cell M(i,k) */
  int     r     = (k-1) / Q;
  __m256 *tp    = om->tfv_avx + 7*q;       	/* *tp now at start of transitions to cur cell M(i,k) */
