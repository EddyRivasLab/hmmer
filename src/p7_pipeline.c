...
float
myget_scthresh(char* name, const ESL_DSQ *dsq, int L, P7_PROFILE *gm, float nu, P7_BG *bg, double P) {
	  int 	  	   status;
	  float        tloop = logf((float) gm->max_length / (float) (gm->max_length+3));
	  float        tmove = logf(     3.0f / (float) (gm->max_length+3));
	  float        tbmk  = logf(     2.0f / ((float) gm->M * (float) (gm->M+1)));
		  ...
		  if (max_run_sc > sc_thresh) {
			  for (i = max_run_i; i<max_run_j; i++) {
				  ...
				  for (j = max_run_j; j>=i; j--) {
					  ...
					  if (run_sc > sc_thresh) {
						  if (min_ratio > best_ratio) best_ratio = min_ratio;
						  j = i-1; // done with that prefix - I hit the threshold.
					  }
				  }
...
float
myget_maxdrop_ssv_hit(char* name, const ESL_DSQ *dsq, int L, P7_PROFILE *gm, float nu, P7_BG *bg, double P) {
	  int 	  	   status;
	  float        tloop = logf((float) gm->max_length / (float) (gm->max_length+3));
	  float        tmove = logf(     3.0f / (float) (gm->max_length+3));
	  float        tbmk  = logf(     2.0f / ((float) gm->M * (float) (gm->M+1)));
	  ...
	  for (k = 1-gm->L+1; k <= gm->M; k++) { // make sure I scan through the whole 2d table
		  ...
		  // Now I have the maximum scoring run on the diagonal.
		  // For each possible starting position that gives sc above sc_thresh,
		  //  run along the diagonal, and capture the largest-scale drop in score.
		  // keep the smallest-magnitude such value among all diagonals, and return it

		  if (max_run_sc > sc_thresh) {
			  for (i = max_run_i; i<max_run_j; i++) {
				  run_sc = 0;
				  float drop_run_sc = 0.0;
				  float max_drop_loc = 0.0;
				  for (j = i; j<=max_run_j; j++) {
					  sc = scores[j][j+k-1];
					  run_sc += sc;
					  drop_run_sc += sc;

					  if (drop_run_sc < max_drop_loc)
						  max_drop_loc = drop_run_sc; //maximum drop observed in the current i/j run, whether it meets threshold or not

					  if (run_sc > sc_thresh) {
						  if (max_drop_loc > minmax_drop)
							  minmax_drop = max_drop_loc;  //smallest-magnitude maximum drop observed in the current i/j run, if that run meets threshold
					  }
					  if (drop_run_sc>0)
						  drop_run_sc = 0;
				  }
...
int
myget_maxnonpos_len_ssv_hit(char* name, const ESL_DSQ *dsq, int L, P7_PROFILE *gm, float nu, P7_BG *bg, double P) {
	  int 	  	   status;
	  float        tloop = logf((float) gm->max_length / (float) (gm->max_length+3));
	  float        tmove = logf(     3.0f / (float) (gm->max_length+3));
	  float        tbmk  = logf(     2.0f / ((float) gm->M * (float) (gm->M+1)));
	  ...
	  for (k = 1-gm->L+1; k <= gm->M; k++) { // make sure I scan through the whole 2d table
		  ...
		  // Now I have the maximum scoring run on the diagonal.
		  // For each possible starting position that gives sc above sc_thresh,
		  //  run along the diagonal, and capture the longest non-positive run.
		  // Keep the smallest-magnitude such value among all diagonals, and return it
		  int neg_start;
		  int neg_end;
		  float neg_run_sc;
		  if (max_run_sc > sc_thresh) {
			  for (i = max_run_i; i<max_run_j; i++) {
				  ...
				  int max_neg_start=-1;
				  int max_neg_end=-1;
				  float max_neg_run_sc;
				  for (j = i; j<=max_run_j; j++) {
					  sc = scores[j][j+k-1];
					  run_sc += sc;

					  if (neg_run_sc < 0 || sc < 0 )
						  neg_run_sc += sc;
					  ...
					  if (neg_run_sc < 0) {
						  ...
						  if (neg_end-neg_start+1 > max_neg_len) {
							  max_neg_len = neg_end-neg_start+1;
							  max_neg_run_sc = neg_run_sc;
							  max_neg_start = neg_start;
							  max_neg_end = neg_end;
						  }
					  } else {
						  neg_start = j+1;
					  ...
					  if (run_sc > sc_thresh) {
						  if (max_neg_len < shortest_neg_run)
							  shortest_neg_run = max_neg_len;
					  }
...
int
myget_maxpos_len_ssv_hit(char* name, const ESL_DSQ *dsq, int L, P7_PROFILE *gm, float nu, P7_BG *bg, double P) {
	  int 	  	   status;
	  float        tloop = logf((float) gm->max_length / (float) (gm->max_length+3));
	  float        tmove = logf(     3.0f / (float) (gm->max_length+3));
	  float        tbmk  = logf(     2.0f / ((float) gm->M * (float) (gm->M+1)));
	  ...
	  for (k = 1-gm->L+1; k <= gm->M; k++) { // make sure I scan through the whole 2d table
		  ...
		  for (j = i; j<=L && j+k-1<=gm->M; j++)
		  {
			  sc = scores[j][j+k-1];  //[target_pos][model_pos]
			  run_sc += sc;
			  if (run_sc > max_run_sc) {
				  max_run_sc = run_sc;
				  max_run_i = i;
		  ...
		  // Now I have the maximum scoring run on the diagonal.
		  // For each possible starting position that gives sc above sc_thresh,
		  //  run along the diagonal, and capture the longest run of positive scores.
		  // Keep the smallest-magnitude such value among all diagonals, and return it
		  int curr_pos_run_length = 0;

		  if (max_run_sc > sc_thresh) {

			  for (i = max_run_i; i<max_run_j; i++) {
				  run_sc = 0;
				  curr_pos_run_length = 0;
				  int max_pos_run_length_loc = 0;
				  for (j = i; j<=max_run_j; j++) {
					  sc = scores[j][j+k-1];
					  run_sc += sc;
					  ...
					  if (curr_pos_run_length > max_pos_run_length_loc)
												  max_pos_run_length_loc = curr_pos_run_length;

					  if (run_sc > sc_thresh) {
						  if (max_pos_run_length_loc > max_pos_run_length)
							  max_pos_run_length = max_pos_run_length_loc;
					  }
...
p7_Pipeline_LongTarget(P7_PIPELINE *pli, P7_OPROFILE *om, P7_BG *bg, const ESL_SQ *sq, P7_TOPHITS *hitlist)
{
  ...
  float invP = esl_gumbel_invsurv(0.5, om->evparam[p7_MMU],  om->evparam[p7_MLAMBDA]);
  p7_bg_SetLength(bg, om->max_length);
  p7_ReconfigLength(om, om->max_length);
  p7_bg_NullOne  (bg, sq->dsq, om->max_length, &nullsc);
  float sc_thresh =   nullsc  + (invP * eslCONST_LOG2) - tmove - tloop_total - tmove - tbmk - tec;

//printf ("M_v_sc,%s,%d,%.2f\n",om->name, om->M, sc_thresh);
//exit(0);
  ...
  for (i=0; i<hit_cnt; i++){
      ...
      float pct = 0.5;
      int shortest = myget_shortest_ssv_hit(sq->name, subseq, window_len, om, 2.0, bg, pct);
      float minmax_drop = myget_maxdrop_ssv_hit(sq->name, subseq, window_len, om, 2.0, bg, pct);
      int minmax_neglen = myget_maxnonpos_len_ssv_hit(sq->name, subseq, window_len, om, 2.0, bg, pct);
      int max_poslen = myget_maxpos_len_ssv_hit(sq->name, subseq, window_len, om, 2.0, bg, pct);
      float minratio = myget_minratio_ssv_hit(sq->name, subseq, window_len, om, 2.0, bg, pct);

      float scthresh = myget_scthresh(sq->name, subseq, window_len, om, 2.0, bg, pct);

      printf("shortest   : %-20s %d %d\n", sq->name, om->M, shortest);
      printf("drop       : %-20s %d %5.2f\n",sq->name, om->M, minmax_drop);
      printf("maxneglen  : %-20s %d %d\n",sq->name, om->M, minmax_neglen);
      printf("maxposlen  : %-20s %d %d\n",sq->name, om->M, max_poslen);
      printf("minratio   : %-20s %d %5.2f (%6.2f)\n",sq->name, om->M, minratio, scthresh);
      printf("minratiopct: %-20s %d %6.3f\n",sq->name, om->M, minratio/scthresh);
