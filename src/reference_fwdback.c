 ...
 * 2. Negative expectation: average Forward score is negative.
 *
 * Works for any mode, any random sequence length.
 * This is not a particularly strong test for correctness. 
 */
static void
utest_randomseq()
{
 ...
 * p7_ReferenceForward() with a model configured in local-only mode.
 *
 ...
 * Score each sequence, using p7_GForward(dual) and p7_ReferenceForward(local). 
  ...
  for (idx = 0; idx < nseq; idx++)
    {
      ...
      if ( p7_ReferenceForward(dsq, L, gml, rmx, &sc2)   != eslOK) esl_fatal(msg);
