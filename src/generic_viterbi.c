...
{
  ESL_GETOPTS    *go      = p7_CreateDefaultApp("generic_viterbi_benchmark", options, 1, argc, argv, banner, usage);
...
main(int argc, char **argv)
{
  ESL_GETOPTS    *go    = p7_CreateDefaultApp("generic_viterbi_utest", options, 0, argc, argv, banner, usage);
  ESL_RANDOMNESS *r    = esl_randomness_CreateFast(esl_opt_GetInteger(go, "-s"));
  ESL_ALPHABET   *abc  = NULL;
  P7_HMM         *hmm  = NULL;
  P7_PROFILE     *gm   = NULL;
...
main(int argc, char **argv)
{
  ESL_GETOPTS    *go      = p7_CreateDefaultApp("generic_viterbi_example", options, 2, argc, argv, banner, usage);
  char           *hmmfile = esl_opt_GetArg(go, 1);
