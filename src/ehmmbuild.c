...
static ESL_OPTIONS options[] = {
  ...
  { "--noevo_msv",  eslARG_NONE,   FALSE, NULL, NULL,    NULL,  NULL,  NULL,            "do not evolve MSV filter",                                     3 },
  { "--noevo_vit",  eslARG_NONE,   FALSE, NULL, NULL,    NULL,  NULL,  NULL,            "do not evolve VIT filter",                                     3 },
  { "--noevo_fwd",  eslARG_NONE,   FALSE, NULL, NULL,    NULL,  NULL,  "--mx,--mxfile", "do not evolve FWD parser",                                     3 },
  ...
  { "--tol",        eslARG_REAL,  "1e-5", NULL, NULL,    NULL,  NULL,  NULL,            "tolerance",                                                   12 },
...
usual_master(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  if (!esl_opt_IsOn(go, "--noevo_fwd")) {
    P7_BG  *bg = p7_bg_Create(cfg->abc);
    double *f  = NULL;
    int     k;
    
    ESL_ALLOC(f, sizeof(double) * cfg->abc->K);
...
mpi_worker(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
  ...
  noevo_msv  = esl_opt_GetBoolean(go, "--noevo_msv");
  noevo_vit  = esl_opt_GetBoolean(go, "--noevo_vit");
  noevo_fwd  = esl_opt_GetBoolean(go, "--noevo_fwd");
  ...
  while (esl_msa_MPIRecv(0, 0, MPI_COMM_WORLD, cfg->abc, &wbuf, &wn, &msa) == eslOK) 
    {
      ...
      if ( msa->nseq == 1 && esl_opt_IsUsed(go, "--singlemx")) {
        //for, single sequence, use blosum mauitrix:
        sq = esl_sq_CreateDigital(cfg->abc);
        if ((status = esl_sq_FetchFromMSA(msa, 0, &sq)) != eslOK) { strcpy(errmsg, bld->errbuf); goto ERROR; }
        if ((status = p7_EvoSingleBuilder(bld, sq, bg, emR, evomodel, betainf, &hmm, NULL, NULL, NULL, noevo_msv, noevo_vit, noevo_fwd, fixtime, tol)) != eslOK)
	  {
	    strcpy(errmsg, bld->errbuf); goto ERROR;
	  }
        esl_sq_Destroy(sq);
        sq = NULL;
        hmm->eff_nseq = 1;
      } else {
        if ((status = p7_EvoBuilder(bld, msa, bg, emR, evomodel, betainf, &hmm, NULL, NULL, NULL, postmsa_p, noevo_msv, noevo_vit, noevo_fwd, fixtime, tol)) != eslOK)
	  {
	    strcpy(errmsg, bld->errbuf); goto ERROR;
	  }
      }
