  ...
  while (strncmp(seq, "//", 2) != 0) {
    ...
    if (*ptr && strncmp(ptr, "//", 2) != 0) {
      ...
      if (status == eslOK) {
        ...
        if (sctp_sendmsg(sock, seq, n, (struct sockaddr *)&serv_addr, sizeof(serv_addr), 0, 0, 0, 0, 0) != n) {
          fprintf(stderr, "%s: sctp_sendmsg (size %d) error %d - %s\n", argv[0], n, errno, strerror(errno));
          exit(1);
        }
        ...
        if ((size = sctp_recvmsg(sock, &stats, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
        ...
        for (i = 0; i < stats.nhits; ++i) {
          ...
          total += n;
          len = sizeof(peer_addr);
          if ((size = sctp_recvmsg(sock, hit, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
            fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
            exit(1);
          }
          ...
          if (hit->name != NULL) {
            ...
            hit->name = malloc(n);
            len = sizeof(peer_addr);
            if ((size = sctp_recvmsg(sock, hit->name, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
              fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
              exit(1);
            }
          }
          if (hit->acc != NULL) {
            ...
            hit->acc = malloc(n);
            len = sizeof(peer_addr);
            if ((size = sctp_recvmsg(sock, hit->acc, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
              fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
              exit(1);
            }
          }
          if (hit->desc != NULL) {
            ...
            hit->desc = malloc(n);
            len = sizeof(peer_addr);
            if ((size = sctp_recvmsg(sock, hit->desc, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
              fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
              exit(1);
            }
          }
          ...
          for (j = 0; j < hit->ndom; ++j) {
            ...
            total += n;
            len = sizeof(peer_addr);
            if ((size = sctp_recvmsg(sock, dcl, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
              fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
              exit(1);
            }
            ...
            total += n;
            len = sizeof(peer_addr);
            if ((size = sctp_recvmsg(sock, ad, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
              fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
              exit(1);
            }
            ...
            total += n;
            len = sizeof(peer_addr);
            if ((size = sctp_recvmsg(sock, ad->mem, n, (struct sockaddr *)&peer_addr, &len, &info, &flags)) == -1) {
              fprintf(stderr, "%s: sctp_recvmsg error %d - %s\n", argv[0], errno, strerror(errno));
              exit(1);
            }
