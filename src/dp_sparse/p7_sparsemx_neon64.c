 ...
 */
P7_SPARSEMASK *
p7_sparsemask_Create_neon64(int M, int L)
{
#ifdef HAVE_NEON64  
 ...
 */
int
p7_sparsemask_Reinit_neon64(P7_SPARSEMASK *sm, int M, int L)
{
 ...
 */
size_t
p7_sparsemask_Sizeof_neon64(const P7_SPARSEMASK *sm)
{
 ...
 */
size_t
p7_sparsemask_MinSizeof_neon64(const P7_SPARSEMASK *sm)
{
  size_t n = sizeof(P7_SPARSEMASK);
#ifdef HAVE_NEON64 
 ...
 */
void
p7_sparsemask_Destroy_neon64(P7_SPARSEMASK *sm)
{
  if (sm) {
#ifdef HAVE_NEON64    
...
int
p7_sparsemask_StartRow_neon64(P7_SPARSEMASK *sm, int i)
{
#ifdef HAVE_NEON64  
  int r;
  int status;
 ...
 */
 int
p7_sparsemask_Add_neon64(P7_SPARSEMASK *sm, int q, int r)
{
#ifdef HAVE_NEON64  
  int     k = r*sm->Q+q+1;
...
int
p7_sparsemask_FinishRow_neon64(P7_SPARSEMASK *sm)
{
#ifdef HAVE_NEON64  
  int *p;
  int  r;
...
int
p7_sparsemask_Finish_neon64(P7_SPARSEMASK *sm)
{
 #ifdef HAVE_NEON64
  int i,r;
  int s;
...
int
p7_sparsemask_Dump_neon64(FILE *ofp, P7_SPARSEMASK *sm)
{
#ifdef  HAVE_NEON64
  int i,k,z;
 ...
 */
int
p7_sparsemask_Compare_neon64(const P7_SPARSEMASK *sm1, const P7_SPARSEMASK *sm2)
{
  #ifdef HAVE_NEON64
 ...
 */
int
p7_sparsemask_Validate_neon64(const P7_SPARSEMASK *sm, char *errbuf)
{
#ifdef HAVE_NEON64  
  int g, i;
 ...
 */
int
p7_sparsemask_SetFromTrace_neon64(P7_SPARSEMASK *sm, ESL_RANDOMNESS *rng, const P7_TRACE *tr)
{
#ifdef HAVE_NEON64
  float cellprob = 0.5;
  float rowprob  = 0.2;
 ...
 */
static int
validate_dimensions_neon64(const P7_SPARSEMX *sx, char *errbuf)
{
...
static int
validate_no_nan_neon64(const P7_SPARSEMX *sx, char *errbuf)
{
  const P7_SPARSEMASK *sm  = sx->sm;
  float         *dpc = sx->dp;
  float         *xc  = sx->xmx;
...
static int
validate_fwdvit_neon64(const P7_SPARSEMX *sx, char *errbuf)
{
  const P7_SPARSEMASK *sm  = sx->sm;
  float         *dpc = sx->dp;
  float         *xc  = sx->xmx;
...
static int
validate_backward_neon64(const P7_SPARSEMX *sx, char *errbuf)
{
  const P7_SPARSEMASK *sm     = sx->sm;
...
static int
validate_decoding_neon64(const P7_SPARSEMX *sx, char *errbuf)
{
  const P7_SPARSEMASK *sm  = sx->sm;
  float         *dpc = sx->dp;
  float         *xc  = sx->xmx;
 ...
 */
int
p7_sparsemx_Validate_neon64(const P7_SPARSEMX *sx, char *errbuf)
{
  int status;

  if (errbuf) errbuf[0] = '\0';

  if ( (status = validate_dimensions_neon64(sx, errbuf)) != eslOK) return status;
  if ( (status = validate_no_nan_neon64    (sx, errbuf)) != eslOK) return status;

  switch (sx->type) {
  case p7S_UNSET:      ESL_FAIL(eslFAIL, errbuf, "validating an unset sparse DP matrix? probably not what you meant");
  case p7S_FORWARD:    if ( (status = validate_fwdvit_neon64  (sx, errbuf)) != eslOK) return status; break;
  case p7S_BACKWARD:   if ( (status = validate_backward_neon64(sx, errbuf)) != eslOK) return status; break;
  case p7S_DECODING:   if ( (status = validate_decoding_neon64(sx, errbuf)) != eslOK) return status; break;
  case p7S_VITERBI:    if ( (status = validate_fwdvit_neon64  (sx, errbuf)) != eslOK) return status; break;
