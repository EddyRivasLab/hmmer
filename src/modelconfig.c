...
P7ReconfigLength(struct plan7_s *hmm, int L)
{
 ...
 /* We must recompute custom log-odds form, since base log-odds
  * form has changed.  
  *
  * NOTE: if this code is called frequently, then the custom
  * code should have a separate function to copy over just the xsc
  * values, since those are all that we change in this function.
  * As it stands, we recompute the custom tsc/msc/isc parameters too.
  */
 FillCustomLogodds(hmm);
}
