...
static inline int get_starprofile(const P7_BG *bg, const P7_HMM *hmm, P7_PROFILE *gm, P7_OPROFILE *om, P7_HMM **ret_ehmm);
		...
		int noevo_msv, int noevo_vit, int noevo_fwd, float fixtime, float tol)
{
  ...
  if ((status = p7_EvoMSVMu    (r, R, hmm, gm, om, bg, EmL, EmN, lambda,      &mmu, noevo_msv, fixtime, tol)) != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine msv mu");
  if ((status = p7_EvoViterbiMu(r, R, hmm, gm, om, bg, EvL, EvN, lambda,      &vmu, noevo_vit, fixtime, tol)) != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine vit mu");
  if ((status = p7_EvoTau      (r, R, hmm, gm, om, bg, EfL, EfN, lambda, Eft, &tau, noevo_fwd, fixtime, tol)) != eslOK) ESL_XFAIL(status,  errbuf, "failed to determine fwd tau");
...
p7_EvoMSVMu(ESL_RANDOMNESS *r, P7_RATE *R, P7_HMM *hmm, P7_PROFILE *gm, P7_OPROFILE *om, P7_BG *bg,
	    int L, int N, double lambda, double *ret_mmu, int noevo, float fixtime, float tol)
{
  ...
  for (i = 0; i < N; i++)
    {
      ...
      // initilize the hmm and profiles to time_star
      get_starprofile(bg, hmm, gm, om, &ehmm);
      
      time = time_star;
      status = p7_OptimizeMSVFilter(dsq, L, &time, R, hmm, gm, om, bg, ox, &sc, noevo, fixtime, tol);
  ...
  *ret_mmu = 0.0;
  ...
  if (ehmm != NULL) p7_hmm_Destroy(ehmm);
  return status;
...
p7_EvoViterbiMu(ESL_RANDOMNESS *r, P7_RATE *R, P7_HMM *hmm, P7_PROFILE *gm, P7_OPROFILE *om, P7_BG *bg,
		int L, int N, double lambda, double *ret_vmu, int noevo, float fixtime, float tol)
{
  P7_OMX  *ox        = p7_omx_Create(om->M, 0, 0); /* DP matrix: 1 row version */
  P7_HMM  *ehmm      = NULL;
  ESL_DSQ *dsq       = NULL;
  ...
  for (i = 0; i < N; i++)
    {
     ...
     // initilize the hmm and profiles to time_star
      ...
      time = time_star;
      status = p7_OptimizeViterbiFilter(dsq, L, &time, R, hmm, gm, om, bg, ox, &sc, noevo, fixtime, tol);
      if (status == eslERANGE) { sc = maxsc; status = eslOK; }
      if (status != eslOK)     goto ERROR;
...
p7_EvoTau(ESL_RANDOMNESS *r, P7_RATE *R, P7_HMM *hmm, P7_PROFILE *gm, P7_OPROFILE *om, P7_BG *bg, 
	  int L, int N, double lambda, double tailp, double *ret_tau, int noevo, float fixtime, float tol)
{
  ...
  for (i = 0; i < N; i++)
    {
      ...
      time = time_star;
      if ((status = p7_OptimizeForwardParser(dsq, L, &time, R, hmm, gm, om, bg, ox, &fsc, noevo, fixtime, tol)) != eslOK) goto ERROR;
      if ((status = p7_bg_NullOne(bg, dsq, L, &nullsc))                                                         != eslOK) goto ERROR;   
...
static inline int
get_starprofile(const P7_BG *bg, const P7_HMM *hmm, P7_PROFILE *gm, P7_OPROFILE *om, P7_HMM **ret_ehmm)
{  
  P7_HMM  *ehmm = NULL;
  int      status;
...
main(int argc, char **argv)
{
  ...
  while ((status = p7_hmmfile_Read(hfp, &abc, &hmm)) != eslEOF) 
	...
	{
	  if (do_msv) p7_EvoMSVMu     (r, om, bg, EmL, EmN, lambda,       &mmu);
	  if (do_vit) p7_EvoViterbiMu (r, om, bg, EvL, EvN, lambda,       &vmu);
	  if (do_fwd) p7_EvoTau       (r, om, bg, EfL, EfN, lambda, Eft,  &ftau);
...
main(int argc, char **argv)
{
  ...
  while (N--)
    { /*                cfg   rng   bg    gm    om  */
      p7_EvoCalibrate(hmm, NULL, NULL, NULL, NULL, NULL);
    }
