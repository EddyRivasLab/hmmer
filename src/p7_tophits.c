...
p7_tophits_Targets(FILE *ofp, P7_TOPHITS *th, P7_PIPELINE *pli, int textw)
{
  ...
  { 
      ...
      if ( th->N > 0 && th->hit[0]->ndom > 0 ) {
          if (pli->mode == p7_SEARCH_SEQS && pli->is_translated)
          {
          ...
          else if (pli->mode == p7_SCAN_MODELS && pli->is_translated)
          {
  ...
  for (h = 0; h < th->N; h++)
    if (th->hit[h]->flags & p7_IS_REPORTED)    
    {
        ...
        else
        {
          if (pli->is_translated)
	   {
                if (fprintf(ofp, "%-*s %-*s %-*s %-*s %-*s %9.2g %6.1f %5.1f %9.2g %6.1f %5.1f %5.1f %3d %3d %3d %3d %3d %3d %3d %s\n",
                tnamew, th->hit[h]->name,
                taccw,  th->hit[h]->acc ? th->hit[h]->acc : "-",
                qnamew, qname,
                qaccw,  ( (qacc != NULL && qacc[0] != '\0') ? qacc : "-"),
