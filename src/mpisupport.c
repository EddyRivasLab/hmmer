...
/* Function:  p7_profile_MPIPackSize()
 * Synopsis:  Calculates size needed to pack a profile.
 *
 * Purpose:   Calculate an upper bound on the number of bytes
 *            that <p7_pcoi9_MPIPack()> will need to pack a profile
 *            <gm> in a packed MPI message for MPI communicator
 *            <comm>; return that number of bytes in <*ret_n>.
 *
 * Returns:   <eslOK> on success, and <*ret_n> contains the answer.
 *
...
int p7_profile_MPIPackSize(P7_PROFILE *gm, MPI_Comm comm, int *ret_n){
  ...
  if (gm == NULL) { 
    ...
    if (MPI_Pack_size(                  p7_MAXABET, MPI_FLOAT, comm, &sz) != 0)    ESL_XEXCEPTION(eslESYS, "pack size failed");  else n += sz;   /* composition */
  }
...
/* Function:  p7_profile_MPIPack()
 * Synopsis:  Packs a profile into MPI buffer.
 *
 * Purpose:   Packs profile <gm> into an MPI packed message buffer <buf>
 *            of length <n> bytes, starting at byte position <*position>,
 *            for MPI communicator <comm>.
 *            
 *            The caller must know that <buf>'s allocation of <n>
 *            bytes is large enough to append the packed HMM at
 *            position <*pos>. This typically requires a call to
 *            <p7_profile_MPIPackSize()> first, and reallocation if
 *            needed.
 *            
 * Returns:   <eslOK> on success; <buf> now contains the
 *            packed <hmm>, and <*position> is set to the byte
 ...
 */
int
p7_profile_MPIPack(P7_PROFILE *gm, char *buf, int n, int *position, MPI_Comm comm)
{
...
p7_profile_MPISend(P7_PROFILE *gm, int dest, int tag, MPI_Comm comm, char **buf, int *nalloc)
{
  ...
  position = 0;
  if(p7_profile_MPIPack(gm, *buf, n, &position, comm) != eslOK){
        ESL_XEXCEPTION(eslESYS, "packing profile failed");
  }
...
/* Function:  p7_profile_MPIUnpack()
 * Synopsis:  Unpacks a profile from an MPI buffer.
 *
 * Purpose:   Unpack a newly allocated profile from MPI packed buffer
 *            <buf>, starting from position <*pos>, where the total length
 *            of the buffer in bytes is <n>. 
 *           
 *
 * Returns:   <eslOK> on success. <*pos> is updated to the position of
 *            the next element in <buf> to unpack (if any). <*ret_profile>
...
p7_profile_MPIRecv(int source, int tag, MPI_Comm comm, const ESL_ALPHABET *abc, const P7_BG *bg, char **buf, int *nalloc,  P7_PROFILE **ret_gm)
{
  ...
  position = 0;
  if(p7_profile_MPIUnpack(*buf, n, &position, comm, (ESL_ALPHABET **) &abc, &gm) != eslOK){
...
{
  ESL_GETOPTS    *go      = p7_CreateDefaultApp("mpisupport_benchmark", options, 1, argc, argv, banner, usage);
...
/* note: this utest will typically fail if run under gdb, because it requires the master 
    and worker's HMMS to have been created in the same second and thus have the same 
    creation time.
*/
static void
...
{
  ESL_GETOPTS *go = p7_CreateDefaultApp("mpisupport_utest", options, 0, argc, argv, banner, usage);
  int          stalling = FALSE;
  int          my_rank;
  int          nproc;
