...
/* Function: ForwardScoreCorrection()
 * Date:     Alex Coventry, Wed Apr 12 13:52:15 2006 [StL]
 * 
 * Purpose: Calculate a correction (in integer log_2 odds) to be
 *           applied to the forward score of a sequence, using a
 *           second null model, based on sampled tracebacks. Do this
 *           by averaging the "null2" correction used in Viterbi over
 *           the posterior distribution on the alignments.
 *           
 * Return:   the log_2-odds score correction.          
 */
float
...
#define NUMALGSAMPLES 10
  ...
  /* Compute and return the average of these scores, first converting
     to exponentials to take the sum, then converting back to
     scores. */
  escores = 0;
  for (sampidx = 0; sampidx < NUMALGSAMPLES; sampidx++) {
